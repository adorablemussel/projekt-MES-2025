...
int main(int argc, char *argv[])
{
    ...
    char* inputPath = "grids/Test2_4_4_MixGrid.txt"; // trzeba sprawdzić czy jest w build/.../debug
    ...
    for (int i = 0; i < 9; i++){
        ...
        mesh.elements[i].calculateHMatrix(shapeFuns, globalData);
        mesh.elements[i].print();

        cout << "Calka powierzchniowa: ";
        if (NPC == 4){
            cout << GaussTable2pt::integral2D_global(gaussFunction2, shapeFuns, mesh.elements[i]) << endl;
            cout << endl;
        }
        else if (NPC == 9){
            cout << GaussTable3pt::integral2D_global(gaussFunction2, shapeFuns, mesh.elements[i]) << endl;
            cout << endl;
        }
        ...
        {
            const auto &H_elem = mesh.elements[i].H_local[0].H_elem;
            for (int r = 0; r < 4; r++){
                for (int c = 0; c < 4; c++){
                    cout << H_elem[r][c] << " ";
                }
